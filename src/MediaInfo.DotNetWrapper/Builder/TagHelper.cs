        ...
        private static readonly List<Tuple<string, ParseDelegate<object>>> TagItems = new List<Tuple<string, ParseDelegate<object>>>
    {
      ...
      new Tuple<string, ParseDelegate<object>>("Cover_Data", TryGetString),
        ...
        public static T GetAllTags<T>(MediaInfo mediaInfo, StreamKind kind, int streamPosition) where T : BaseTags, new()
        {
            ...
            // parse covers
            result.Tags.TryGetValue("Cover_Type", out object coverType);
            result.Tags.TryGetValue("Cover_Mime", out object coverMime);
            result.Tags.TryGetValue("Cover_Description", out object coverDescription);
            result.Tags.TryGetValue("Cover", out object cover);
            result.Tags.TryGetValue("Cover_Data", out object coverData);

            if (coverData != null || coverType != null || coverMime != null || coverDescription != null || cover != null)
            {
        ...
        {
            var result = int.TryParse(source, out int resultValue);
            value = resultValue;
            return result;
        }
        ...
        {
            var result = double.TryParse(source, out double resultValue);
            value = resultValue;
            return result;
        }
